...
#
# Input:
#   Input/*.dta — 138 per-country Stata data files published by Busch et al. as the
#     replication dataset for the 2024 paper. Not generated by any script in this repo;
#     download from the paper's data repository. Each file contains one row per pixel with
...
#   Output/choose_harvest_years_mapping.csv — produced by 0_choose_harvest_years.py (Step 1
#     of the pipeline). Maps every candidate rotation year to its chosen bucket representative;
#     must exist before this script is run.
...
# create_database.py in the SMDAMAGE project reads Busch2024_to_SMDAMAGE.sqlite and loads
# forestry bidders — one per (rotation_year, cluster_index) — into the SMDAMAGE auction.
...
# from the Faustmann rule to a smaller set of representative years. 
...
# Then calculate_harvest_year() for each pixel's series. 
