...
CONFIG = RunConfig(
	# db_file=OUTPUT_DIR / "Busch2024_to_SMDAMAGE.sqlite",
	# k_values=[20],
	# rotation_year=5,
	# max_years=30,
	# profile_source="pixel",
	# load_method="pivot",
	# max_iter=200,
	# tol=1e-4,
	# seed=0,
	# normalize=False,
	# write_to_CSV=True,
	# output_prefix=OUTPUT_DIR / "k_means_carbon_removal",
)
...
def write_schedule_results_to_db(con: sqlite3.Connection, harvest_year: int, centers: np.ndarray, keys: list[tuple[str, int]], labels: np.ndarray, areas: np.ndarray, counts: np.ndarray, max_years: int) -> tuple[int, int]:
	...
	con.executemany(
		"insert into _cluster_assignments (country, pixel_id, cluster_index) values (?, ?, ?)",
		[(country, int(pixel_id), int(cluster_id)) for (country, pixel_id), cluster_id in zip(keys, labels)],
	)
	con.execute(
		f"""
...
def iter_profiles_for_harvest_year(con: sqlite3.Connection, harvest_year: int, max_years: int, pixel_index: str | None, needs_cast: bool):
	...
	query = f"""
		select p.country, p.pixel_id, p.area_ha, y.year, y.tC_per_ha_per_year
		from {PIXEL_TABLE} as p{index_clause}
		join {YEAR_TABLE} y on y.country = p.country and y.pixel_id = p.pixel_id
		where {where_clause}
		order by p.country, p.pixel_id, y.year
	"""
...
def load_profiles_for_harvest_year_pivot(con: sqlite3.Connection, harvest_year: int, max_years: int, normalize: bool, pixel_index: str | None, needs_cast: bool) -> tuple[list[tuple[str, int]], np.ndarray, np.ndarray]:
	...
	parameter = harvest_year if needs_cast else float(harvest_year)
	year_columns = ", ".join([
		f"coalesce(max(case when y.year = {year} then y.tC_per_ha_per_year end), 0.0) as year_{year}"
		for year in range(1, max_years + 1)
	])
	query = f"""
		select p.country, p.pixel_id, p.area_ha, {year_columns}
		from {PIXEL_TABLE} as p{index_clause}
		left join {YEAR_TABLE} as y
			on y.country = p.country and y.pixel_id = p.pixel_id and y.year between 1 and {max_years}
		where {where_clause}
		group by p.country, p.pixel_id, p.area_ha
		order by p.country, p.pixel_id
	"""
...
def load_schedule_profiles_for_harvest_year(con: sqlite3.Connection, harvest_year: int, max_years: int, normalize: bool) -> tuple[list[tuple[str, int]], np.ndarray, np.ndarray]:
	...
	vectors = []
	year_columns = ", ".join([
		f"coalesce(max(case when year = {year} then tC_per_ha_per_year end), 0.0) as year_{year}"
		for year in range(1, max_years + 1)
	])
	query = f"""
		select cluster_index, max(total_area_ha) as total_area_ha, {year_columns}
		from {SCHEDULE_YEAR_TABLE}
		where selected_rotation_year = ?
...
def write_summary_csv(path: Path, rows: list[dict[str, object]]) -> None:
	fieldnames = [
		"rotation_year", "k", "cluster_id", "pixel_count", "total_area_ha", "iterations_run",
...
def write_overall_csv(path: Path, rows: list[dict[str, object]]) -> None:
	fieldnames = [
		"rotation_year", "k", "pixel_count", "cluster_count", "iterations_run",
		"total_squared_distance", "mean_squared_distance", "rmse_per_year",
...
def k_values_tag(k_values: list[int]) -> str:
	return "_".join([str(int(k)) for k in k_values])
...
if __name__ == "__main__":
	...
	cluster_trees(rotation_year=6, k=[12], write_to_CSV=True) 
	cluster_trees(rotation_year=10, k=[32], write_to_CSV=True)
	cluster_trees(rotation_year=17, k=[20], write_to_CSV=True)
	cluster_trees(rotation_year=35, k=[7], write_to_CSV=True)